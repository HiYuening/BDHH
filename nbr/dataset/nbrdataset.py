...
class NBRDataset(Dataset):
    def __init__(self, corpus, mode):
        ...
        if self.mode == "train":
            ...
            self.neg_samples = [np.array(list(set(np.arange(self.corpus.n_items)).difference(set(consumed_items[user_id])))) for user_id in tqdm(range(corpus.n_users))]#这段代码的目的是在训练模式下为每个用户生成负样本，即用户没有购买过的商品编号。
        if self.mode in ["dev", "test"]:
            ...
            for index in tqdm(range(len(self.corpus.data[self.mode]))):
                ...
                t = self.corpus.book[user_id][orders[0]].time / TIME_SCALAR#单位（如天）
                self.ts.append(t)
                self.history_times.append({})
                for click in self.corpus.book[user_id]:
                    if click.time >= self.corpus.book[user_id][orders[0]].time:#历史时间要小于验证集的最小时间
                        break
